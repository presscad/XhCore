...
{
	for(int i=0;i<globalAngleLibrary.Count;i++)
	{
		if(fabs(globalAngleLibrary[i].wing_wide-wing_wide)<0.5)
			return globalAngleLibrary[i].r;
	}
	return 0;
}
...
BOOL QuerySteelMatMark(char briefmark,char* steelmark,int niMaxStrBuffLen/*=8*/)
{
	int i;
	for(i=0;i<steelmat_N;i++)
	{
		if(toupper(steelmat_tbl[i].cBriefMark)==toupper(briefmark))
		{
			if(steelmark)
			{
				CXhString szSteelName(steelmark,niMaxStrBuffLen);
				szSteelName.Copy(steelmat_tbl[i].mark);
				if(steelmat_tbl[i].cPreferQualityLevel>='A')
					szSteelName.Append(steelmat_tbl[i].cPreferQualityLevel);
			}
			return TRUE;
		}
	}
...
JG_PARA_TYPE* FindJgType(double wide,double thick,char cType/*='L'*/)
{
	for(int i=0;i<globalAngleLibrary.Count;i++)
	{
		if(fabs(globalAngleLibrary[i].wing_wide-wide)<EPS&&fabs(globalAngleLibrary[i].wing_thick-thick)<EPS
			&&toupper(globalAngleLibrary[i].cType)==toupper(cType))
			return &globalAngleLibrary[i];
	}
	return NULL;
}
